...
- `docs/persona/` — documentos de Persona (canvas + parecer/dores) gerados por `/fundraiser-test`, `/fundraiser-production-test`, `/fundraiser-submission-timeline` (lente de captador de recursos individual), e por `/coordenador-test`, `/coordenador-production-test` (lente de coordenação/liderança de pesquisa institucional, arquivos com sufixo `-coordenador`). Também guarda `coordenador-instituto.html`, a agenda estratégica canônica do instituto.
...
- `docs/edital-fit/` — pareceres de oportunidade (perseguir/não perseguir um edital) gerados por `/coordenador-edital-fit`, cruzando o texto do edital com a agenda estratégica do instituto e sinal informal fornecido pelo usuário.
...
| `/coordenador-test` | O `coordenador-de-pesquisa` vira a liderança de pesquisa institucional e testa um protótipo estático, devolvendo um documento de Persona com parecer/dores em `docs/persona/` (lente de portfólio/estratégia, não de um único captador) |
| `/coordenador-production-test` | Igual ao anterior, mas testa a aplicação real já implementada |
| `/coordenador-edital-fit` | Avalia se um edital real vale a pena perseguir, cruzando o texto do edital com a agenda estratégica do instituto e sinal informal que você fornecer, devolvendo um parecer em `docs/edital-fit/` |
